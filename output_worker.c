...
int
get_dest(int type, char *src_id, char *dest)
{
    char *typename = NULL;
    char dbuf[128] = {0}; 
    char tbuf[128] = {0};
    ...
    typename = audit_type_array[type].name;
    ...
    sprintf(dest, "%s/%s/%s%03d_"GATHER_TYPE"_%s_%s_%s.log", typename, dbuf, tbuf,
                                ...
                                src_id,
                                cfg->org_code,
                                audit_type_array[type].type);
}

#if 1
int
transfer_file(char *path, int type, char *src_id)
{
    char buf[512] = {0};
    char buf2[512] = {0};
    char dest[128] = {0};
    ...
    */
    ...
    if((ret = ftp_storfile_by_port(sock, path, p, NULL, NULL)) != 0){
        blog(LOG_ERR, "failed to put file to server");
        goto fail;
    }

    sprintf(buf, "%s.ok", p);
    if((ret = ftp_storfile_by_port(sock, OK_FLAG, buf, NULL, NULL)) != 0){
        blog(LOG_ERR, "failed to create ok flag file");
        goto fail;
    }
    ftp_quit(sock);
    blog(LOG_DEBUG, "file %s transfer to server succeed", path);
    dest[p - buf - 1] = 0;
    sprintf(buf, "mkdir -p %s/"DONE_DIR"/%s; mv %s %s/"DONE_DIR"/%s/%s &", cfg->data_path, dest, path, cfg->data_path, dest, dest + (p - buf));
    call_system(buf);
    return 0;
fail:
...
transfer_file(char *path, int type, char *src_id)
{
    ...
    typename = audit_type_array[type];
...
scan_dir(char *path, char *current_name)
{
    ...
    while((entry = readdir(dp)) != NULL) {
        ...
        if(S_ISDIR(statbuf.st_mode)){
            ...
            if(transfer_file(buf, get_audit_id_by_type(current_name), src_id)){
